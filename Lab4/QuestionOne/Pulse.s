...
;---------------------------------------------------
LOW					EQU	0x00000028 ;4x (40us=1us * (0x28))
HIGH				EQU	0x0000000A ;1x (10us=1us * (0x0A))
...
My_Timer0A_Handler	PROC
					LDR		R1,=TIMER0_TAILR;When isr is entered, check the region
					LDR		R2,[R1]
					CMP		R2,HIGH
					BEQ		TurnOff ;If previous one is High, then go into Low mode
					CMP		R2,LOW
					BEQ		TurnOn	;If previous one is Low, then go into High mode
