...
;-------------------------PRINT-------
exit				CMP			R4,#0xFFFFFFFF		;NEGATIVE CHECKER
					BLEQ		minus				;If it is negative then print minus
;R5 = X.YZ
					AND			R4,R5,0xF00			; Take digit X
					LSR			R4,#8
					BL			CONVRT				;Print X
					BL			dot					;Print dot
					AND			R4,R5,0x0F0			;Take digit Y
					LSR			R4,#4
					BL			CONVRT				;Print Y
					AND			R4,R5,0x00F			;Take digit Z
					BL			CONVRT				;Print digit Z
					MOV			R5,#0x20
					BL			OutChar
...
;Clear interrput register			
					...
					STR			R4,[R2]
;Return the cycle
					BL			DELAY100			;Delaying
					B			takeSample			;Looping
